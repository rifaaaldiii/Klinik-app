<div class="tabular__wrapper">
    ...
    <div class="row g-3 mt-2">
        <div class="row">
            <div class="col-md-12 tombol">
                ...
                <button type="button" class="btn btn-sm" id="btnPenggajian" style="border-bottom: none; background-color: transparent; box-shadow: none;" onclick="showSection('penggajian', this)">
                    <p style="font-weight: 600;">Penggajian</p>
                </button>
            </div>
        </div>
    ...
    <div class="row g-3 mt-2">
                    ...
                    <tbody id="transaksiTableBody">
                        ...
                        ?>
                                <tr>
                                    ...
                                    <td>
                                        <a href="pdf/transaksi.php?notrans=<?= $transaksi_row['notrans'] ?>" class="btn btn-sm btn-success" target="_blank">
        ...
        <div id="penggajian" class="section" style="display: none;">
            <form action="pdf/rekap_penggajian.php" method="get">
                <div class="header__title text-sm mb-4" style="flex:1;">
                    <h3 style="font-weight: 600; font-size: 1.2rem; margin-bottom: 0;">Tabel Penggajian</h3>
                    <div class="d-flex justify-content-end mb-4 mt-0" style="gap: 1rem;">
                        <div class="input-group" style="width: 200px;">
                            <input style="border-radius: 12px;" type="month" name="tanggal" id="tanggalFilterPenggajian" class="form-control" placeholder="Bulan & Tahun">
                        </div>
                        <button type="submit" class="btn btn-success">
                            <i class="fa fa-file-pdf"></i> Download PDF
                        </button>
            ...
            <div style="overflow-y: auto; max-height: 340px;">
                <table>
                    ...
                    <tbody id="penggajianTableBody">
                        ...
                        $query_penggajian = mysqli_query($conn, "SELECT p.*, k.nama, g.gaji_pokok
                        FROM penggajian p
                        LEFT JOIN karyawan k ON p.karyawan_id = k.id 
                        LEFT JOIN golongan g ON k.golongan_id = g.id
                        WHERE p.status = 'Completed'");
                        if (mysqli_num_rows($query_penggajian) > 0) {
                            while ($penggajian_row = mysqli_fetch_assoc($query_penggajian)) {
                        ?>
                                <tr>
                                    <td><?= $no++ ?>.</td>
                                    <td><?= $penggajian_row['tanggal'] ?></td>
                                    <td><?= $penggajian_row['nama'] ?></td>
                                    <td>Rp. <?= number_format($penggajian_row['gaji_pokok'], 0, ',', '.') ?></td>
                                    <td>Rp. <?= empty($penggajian_row['asistensi']) ? '0' : number_format($penggajian_row['asistensi'], 0, ',', '.') ?></td>
                                    <td>Rp. <?= empty($penggajian_row['total']) ? '0' : number_format($penggajian_row['total'], 0, ',', '.') ?></td>
                                    <td>
                                        <a href="pdf/penggajian.php?id=<?= $penggajian_row['id'] ?>" class="btn btn-sm btn-success" target="_blank">
                                            <i class="fa fa-print"></i> Print
                                        </a>
                                    </td>
                                </tr>
                    ...
                    <tfoot style="position: sticky; bottom: 0; z-index: 1;">
                        <tr>
                            <td colspan="7">Note : Filter dengan dengan bulan dan tahun</td>
                        </tr>
                    </tfoot>
...
<script>
    document.getElementById('searchInput').addEventListener('keyup', function() {
        ...
        var rows2 = document.querySelectorAll('#penggajianTableBody tr');
        var foundAny = false;
        var foundAny1 = false;
        ...
        rows2.forEach(function(row2) {
            ...
            row2.style.display = found ? '' : 'none';
            if (found) foundAny1 = true;
        });

        handleNoDataMessage('transaksiTableBody', 'noDataRow', foundAny, 6);
        handleNoDataMessage('penggajianTableBody', 'noDataRow1', foundAny1, 7);
    });
    ...
    document.getElementById('tanggalFilterPenggajian').addEventListener('change', function() {
        filterByDate('penggajianTableBody', this.value, 7);
    });
    ...
    document.getElementById('tanggalFilterPenggajian').value = `${year}-${month}`;
</script>